...
export function useFSRS() {
  ...
  const useCreateDeck = () => {
    return useMutation({
      mutationFn: async (deckData: { deck_name: string; description?: string; parent_deck_id?: number }) => {
  ...
  const useDueCards = (deck_id?: number, limit: number = 10) => {
    return useQuery({
      ...
      queryFn: async () => {
        ...
        const response = await fetch(`/api/fsrs?${params}`, {
          headers: getHeaders()
        });
  ...
  const useReviewCard = () => {
    return useMutation({
      mutationFn: async ({ card_id, rating }: { card_id: number; rating: number }) => {
        const response = await fetch('/api/fsrs', {
          method: 'POST',
          headers: getHeaders(),
  ...
  const useCreateFlashcard = () => {
    return useMutation({
      mutationFn: async (cardData: { deck_id: number; front_content: string; back_content: string }) => {
        const response = await fetch('/api/fsrs', {
          method: 'POST',
          headers: getHeaders(),
